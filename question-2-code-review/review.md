...
No método **addUser**, é utilizado o **push** ```bash this.state.users.push(newUser);``` diretamente no array o que também representa uma mutação direta do estado. O correto seria criar um array novo. Sugestão para correção:
...
O método **forceUpdate()** está sendo utilizado para atualizar o componente ```bash this.forceUpdate();```, mas esse método normalmente não deve ser utilizado, pois a interface já é automaticamente atualizada quando o **setState** é chamado. Então, o correto seria remover o método **forceUpdate()**.
